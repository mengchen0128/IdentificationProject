...
def identification_main(mu_temp, PreData_temp, extra_temp):
    ...
    StepTol = extra_temp[0]
    FunTol = extra_temp[1]
    R = int(extra_temp[2])

    # Load pre-processed data, which consists of a structure with Q structs.

    PreData = PreData_temp

    # Programer may tune the follwoing setting as will

    NT = int(extra_temp[3])  # Number of time-periods.
    S_T = int(extra_temp[4])  # Time samples in one time-period.
    C = int(extra_temp[5])
