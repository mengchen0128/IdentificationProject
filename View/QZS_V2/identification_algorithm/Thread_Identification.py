...
class Thread_Identification(QThread):
    ...
    def __init__(self, mu_temp, PreData_temp, extra_temp, *args, **kwargs):
        super(Thread_Identification, self).__init__(*args, **kwargs)
        self.mu_temp = mu_temp
        self.PreData = PreData_temp
    ...
    def run(self):
        ...
        while Iter < R and norm(Gamma) > FunTol and norm(Increment) > StepTol:
            for k in range(0, Q):
                t = linspace(0, NT * 2 * np.pi / self.PreData[k].omega, NT * S_T)
                I = int((np.shape(self.PreData[k].X)[1] - 1) / 2)
                Lambda = DerivativeTransform(self.PreData[k].omega, I)

                x = SignalReconstruction(t, self.PreData[k].omega, self.PreData[k].X)
                dxdt = SignalReconstruction(t, self.PreData[k].omega, self.PreData[k].X @ Lambda)


                g = GoverningFunction(t, dxdt, x, mu)
                G = FourierSeriesExpansion_State(t, g, self.PreData[k].omega, I)

                Gamma1 = self.PreData[k].F[:, 0:1] - G[:, 0:1]
                Gamma2 = self.PreData[k].F[:, 1:2] + (self.PreData[k].omega) ** 2 * M @ (self.PreData[k].X[:, 1:2]) - G[:,
                                                                                                       1:2]
                Gamma3 = self.PreData[k].F[:, 2:3] + (self.PreData[k].omega) ** 2 * M @ (self.PreData[k].X[:, 2:3]) - G[:,
                                                                                                       2:3]
                Gamma_temp = np.concatenate((Gamma1, Gamma2, Gamma3))

                Gamma[k * 3 * N + 0:k * 3 * N + 3 * N] = Gamma_temp

                [Phy_s_0, Phy_c_0] = FourierSeriesExpansion_Grad_Phy(t, dxdt, x, mu, self.PreData[k].omega, 0)
                [Psi_s_0, Psi_c_0] = FourierSeriesExpansion_Grad_Psi(t, dxdt, x, mu, self.PreData[k].omega, 0)

                [Phy_s_1, Phy_c_1] = FourierSeriesExpansion_Grad_Phy(t, dxdt, x, mu, self.PreData[k].omega, 1)
                [Psi_s_1, Psi_c_1] = FourierSeriesExpansion_Grad_Psi(t, dxdt, x, mu, self.PreData[k].omega, 1)
            ...
            for k in range(1, Q + 1):
                self.PreData[k - 1].X[:, 0:1] = self.PreData[k - 1].X[:, 0:1] + Increment[(k - 1) * N:(k - 1) * N + N]
