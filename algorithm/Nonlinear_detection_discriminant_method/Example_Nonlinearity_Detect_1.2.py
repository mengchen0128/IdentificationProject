...
def Example_Nonlinearity_Detect_1():
    ...
    for n in range(0, N_Ome - 1):
        ...
        int2 = functools.partial(CurveFit, omega=Omega[n])
        popt, pcov = curve_fit(int2, t, y, p0=Para0, method='trf')
        Para = popt
        P = 0
    ...
    plt.scatter(Omega, np.array(Nonlinearity))
    plt.xlabel(r"$\omega$")
    plt.ylabel(r"$\rho$")
    plt.show()


def CurveFit(t, *Para, omega):
    # z=numpy.zeros(len(t))
    m = [0] * len(t)
...
def MySystem_Nonlinear(t, y, Para1, Para2):
    ...
    omega = Para2
    dydt = [y[1],
            -2 * c * y[1] - 2 * k * (2 - L / d + L / (2 * d) * (y[0] / d) ** 2) * y[0] + A * np.sin(omega * t) / m]
    return dydt
